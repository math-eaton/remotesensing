...
// Define knots for the piecewise linear approximation
const int numKnots = 4; // You can increase the number of knots for better accuracy
int knotsValues[numKnots] = {0, 300, 512, 1023}; // Raw ADC values read from potentiometer
int knotsPercent[numKnots] = {0, 30, 94, 100}; // Corresponding percentages of travel

// Function to perform linear interpolation and normalization
int normalizePotValue(int rawValue) {
    if (rawValue <= knotsValues[0]) return 0;
    if (rawValue >= knotsValues[numKnots - 1]) return 1023;

    // Find which two knots the rawValue is between
    for (int i = 0; i < numKnots - 1; i++) {
        if (rawValue >= knotsValues[i] && rawValue < knotsValues[i + 1]) {
            // Perform linear interpolation
            float slope = (float)(knotsPercent[i + 1] - knotsPercent[i]) / (knotsValues[i + 1] - knotsValues[i]);
            float intercept = knotsPercent[i] - slope * knotsValues[i];
            float normalizedValue = slope * rawValue + intercept;
            return (int)((normalizedValue / 100.0) * 1023); // Scale percentage to 0-1023
        }
    }
    return -1; // Error case
}
...
void loop() {
  ...
  int normalizedLEDValue = normalizePotValue(LEDpotValue);
